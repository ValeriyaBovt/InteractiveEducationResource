...
public class DeterminantTest {
    ...
    public DeterminantTest() throws IOException {
    }

    private  Question[] question;
    public Question[] fullArray () throws IOException {
        File file = new File("./src/sample/Determinant/question_determinant.txt");
        ...
        int i=0;
        Scanner s = new Scanner(new File("./src/sample/Determinant/question_determinant.txt"));
        while (i<n) {
            ...
            if (selectedRadio!=null) {
                ...
                if (toggleGroupValue.equals(nowCorrectAnswer)) {
                    ...
                    if (nowQuestion + 1 == question.length) {
                        ...
                        // Показуємо текст наприкінці
                        finish_label.setText("   Ви відповіли правильно на   " + correctAnswer + " з " + question.length + " питань!");
                    } else { // Якщо ще є запитання...
                        answerDisable(false);
                        answer_button.setVisible(true);
                        nowQuestion++;// Збільшуємо номер питання
                        nowCorrectAnswer = question[nowQuestion].correctAnswer();// Вказуємо новий текст правильної відповіді
                        question_label.setText(question[nowQuestion].getQuestion()); // Змінюємо текст питання в програмі
                        String[] answers = question[nowQuestion].getAnswer();// отримуємо масив відповідей
                        List<String> intList = Arrays.asList(answers);// Перетворюємо у список (для сортування)
                        Collections.shuffle(intList);// Сортуємо у випадковому порядку
                        // Підставляємо відповіді
                        answer1.setText(intList.get(0));
                        answer2.setText(intList.get(1));
                        answer3.setText(intList.get(2));
                        answer4.setText(intList.get(3));
                        // Знімаємо виділення, що було до цього
                        if (selectedRadio != null) {
